...
def add_courses(db: Session, course_names: list[str]) -> list[Course]:
    ...
    try:
        existing_names = {name for (name,) in db.query(Course.name).filter(Course.name.in_(unique_names)).all()}
...
def add_course_entries_batch(db: Session, entry_requests: list[CourseEntryRequest]) -> list[CourseEntry]:
    if not entry_requests:
        return []
    
    try:
        customer_data = [(item.customer_email, item.customer_name) for item in entry_requests]
        customer_service.add_new_customers(db, customer_data)

        ids = {item.provider_message_id for item in entry_requests}
        existing_ids = {id for (id,) in db.query(CourseEntry.provider_message_id).filter(CourseEntry.provider_message_id.in_(ids)).all()}

        course_names = {item.course_name for item in entry_requests}
        courses_map = {c.name: c for c in db.query(Course).filter(Course.name.in_(course_names)).all()}

        new_entries: list[CourseEntry] = []
        seen_ids = set()
        for item in entry_requests:
            if item.provider_message_id in existing_ids or item.provider_message_id in seen_ids:
                logger.info(f"Skipping duplicate entry with provider_message_id '{item.provider_message_id}'.")
                continue
            ...
            new_entries.append(course_entry)
            seen_ids.add(item.provider_message_id)

        if new_entries:
            db.add_all(new_entries)
            db.commit()
            logger.info(f"Successfully added {len(new_entries)} new course entries to the database.")
